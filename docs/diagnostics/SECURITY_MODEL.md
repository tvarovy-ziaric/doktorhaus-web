...
Mutable client-output storage je mimo webrootu a oddelený od immutable report package. Admin endpoint vyžaduje Admin PIN pri každej operácii, upload MIME určuje server z obsahu a povoľuje iba PDF alebo JPEG/PNG/WebP podľa operácie. Originálny filename ani klientsky path sa nepoužijú. Klientsky output media endpoint autorizuje opaque `outm_…` iba v store inspection recordu naviazaného na aktuálnu diagnostics session; tým blokuje cross-inspection BOLA. SVG, HTML, PHP a neznámy obsah sa do storage neprijmú.

Admin klientsky náhľad je úzka autorizačná výnimka pre interný workflow, nie nový klientsky login. Dedicated POST endpoint prijíma iba Admin PIN a opaque inspection record ID, z recordu serverovo určí access ID, vyžaduje `ready|sent`, aktívny neexpirovaný grant a platný package binding a až potom obnoví štandardnú `DH_DIAGSESSID` cookie. Odpoveď obsahuje iba allowlistovaný redirect; klientsky PIN, Admin PIN, grant obsah ani filesystem path sa nevracajú a operácia sa audituje ako `admin_client_preview_started`.
