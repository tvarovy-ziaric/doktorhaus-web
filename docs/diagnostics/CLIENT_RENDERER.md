...
Mutable output projekcia môže obsahovať viac kariet rovnakého typu, napríklad samostatné Panoraven výstupy pre exteriér a strechu. Názov a opis pochádzajú z adminom spravovaných client-output metadata, URL však stále prechádza typovým HTTPS/host alebo same-origin `outm_…` allowlistom. Pomenované admin galérie sa zobrazujú až v samostatnej sekcii `Doplnková fotodokumentácia`; nemenia počet ani väzby diagnostickej galérie a nedostávajú issue linky.
