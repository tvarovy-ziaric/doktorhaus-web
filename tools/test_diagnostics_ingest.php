...
use DoktorHaus\Diagnostics\MittiImportStore;
...
try{
    ...
    $llm=new DiagnosticsLlmClient($config,$llmTransport);$storage=new DiagnosticsStorage($temporary,__DIR__.'/..');$service=new DiagnosticsIngestService($storage,$config,$mitti,$llm);$imports=new MittiImportStore($storage);
    ...
    $manifest=$imports->loadManifest($job['sourceKey'],$job['sourceRevision']);$snapshot=$temporary.DIRECTORY_SEPARATOR.'imports'.DIRECTORY_SEPARATOR.'mitti'.DIRECTORY_SEPARATOR.$job['sourceKey'].DIRECTORY_SEPARATOR.$job['sourceRevision'];$rawInspectionHash=hash_file('sha256',$snapshot.DIRECTORY_SEPARATOR.'inspection.raw.json');ingest_assert(is_file($snapshot.DIRECTORY_SEPARATOR.'template.raw.json')&&is_file($snapshot.DIRECTORY_SEPARATOR.'answers.raw.ndjson')&&is_string($rawInspectionHash),'raw snapshot files preserved');$storedNames=array_column($manifest['media'],'storage_filename');ingest_assert(count($storedNames)===count(array_unique($storedNames))&&count(array_filter($storedNames,static function(string$name):bool{return preg_match('/^m_[0-9a-f]{24}\.(?:jpg|png|webp|pdf|mp4|mov|webm|bin)$/D',$name)===1;}))===count($storedNames)&&($manifest['media'][0]['original_filename']??'')==='strecha.jpg','malicious and duplicate media filenames handled safely');
    ...
    $output=[];$exit=0;exec('python3 '.escapeshellarg(__DIR__.'/diagnostics_lint.py').' --inspection '.escapeshellarg($inspectionPath).' --diagnosis '.escapeshellarg($diagnosisPath).' 2>&1',$output,$exit);ingest_assert($exit===0,'same diagnostics domain lint as canonical drafts: '.implode(' | ',$output));
    ingest_assert(($service->draft($job['jobId'],'inspection')['id']??null)===$canonical['id']&&($service->draft($job['jobId'],'diagnosis')['inspection_id']??null)===$canonical['id'],'canonical JSON drafts can be downloaded');
    $same=$service->startImport('audit_completed_001');ingest_assert($same['noOp']===true&&hash_file('sha256',$snapshot.DIRECTORY_SEPARATOR.'inspection.raw.json')===$rawInspectionHash,'same raw source import is no-op');$sameNormalized=$service->normalize($same['job']['jobId']);ingest_assert($sameNormalized['noOp']===true,'same canonical re-import is idempotent');
    $edited=$service->draft($job['jobId'],'inspection');$edited['inspection']['limitations'][]='Human edit sentinel.';$replaced=$service->replaceDraft($job['jobId'],'inspection',$edited,(int)$job['storageRevision']);$job=$replaced['job'];ingest_assert($job['state']==='human_edited','corrected inspection JSON upload creates a draft revision');$revalidated=$service->validate($job['jobId']);$job=$revalidated['job'];ingest_assert($revalidated['ok']===true,'corrected JSON validates after human edit');$conflict=$service->startImport('audit_completed_001');$blocked=false;try{$service->normalize($conflict['job']['jobId']);}catch(DiagnosticsIngestException$error){$blocked=$error->getIngestCode()==='IMPORT_HUMAN_CONFLICT';}ingest_assert($blocked,'manual draft is not silently overwritten');
    ...
    $htmlTransport=static function(string$method,string$url,array$headers,$destination,int$limit):array{if(strpos($url,'/media/v1/download/')!==false)return['status'=>200,'headers'=>['Content-Type: application/json'],'body'=>json_encode(['url'=>'https://signed.example.test/html-media'])];return['status'=>200,'headers'=>['Content-Type: text/html'],'body'=>'<html>not media</html>'];};$htmlClient=new MittiClient($config,$htmlTransport);$htmlPath=$temporary.DIRECTORY_SEPARATOR.'html-media.part';$failed=false;try{$htmlClient->downloadMedia('media_html','image',$htmlPath);}catch(DiagnosticsIngestException$error){$failed=$error->getIngestCode()==='MITTI_MEDIA_MIME';}ingest_assert($failed&&!file_exists($htmlPath),'HTML pretending to be a photo is rejected and cleaned up');
    $auditPath=$temporary.DIRECTORY_SEPARATOR.'imports'.DIRECTORY_SEPARATOR.'mitti'.DIRECTORY_SEPARATOR.'audit'.DIRECTORY_SEPARATOR.gmdate('Y-m-d').'.jsonl';$audit=(string)file_get_contents($auditPath);foreach(['mitti_import_started','mitti_import_completed','diagnostic_draft_generated','diagnostic_draft_validated','diagnostic_draft_replaced_by_human']as$event)ingest_assert(strpos($audit,'"event":"'.$event.'"')!==false,'audit event '.$event);ingest_assert(strpos($audit,'synthetic-token')===false&&strpos($audit,'synthetic-openai')===false,'audit excludes API secrets and full content');
